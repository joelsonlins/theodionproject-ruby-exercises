...
def twenty_first_century_years?(year_list)
  ...
  if year_list.all?{|element| element >= 2001 && element <= 2100}
    true
  else
    false
  end
...
def correct_format?(word_list)
  ...
  if word_list.none?{ |element| element == element.upcase }
    true
  else
    false
  end
...
def valid_scores?(score_list, perfect_score)
  ...
  if score_list.one? {|k,v| v==perfect_score}
    true
  else
  false
  end
